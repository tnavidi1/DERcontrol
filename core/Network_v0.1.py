...
class Network(object):
	...
	def __init__(self, storagePen, solarPen, nodesPen, pDemandFull, rDemandFull, pricesFull, root, Ybus, startIdx=0, sNormFull=np.nan, Vmin=0.95, Vmax=1.05, Vtol=0, v_root=1.022, random=True):
		...
		self.storagePen=storagePen, self.solarPen=solarPen, self.nodesPen=nodesPen
		self.rDemandFull=rDemandFull, self.pricesFull=pricesFull
		self.root=root
		self.startIdx=startIdx # starting index of data in pDemandFull and others

		if random==True:
			self.netDemandFull, sGenFull, self.battnodes, self.qmin, self.qmax, self.umin, self.umax = self.setStorageSolar(pDemandFull, sNormFull, storagePen, solarPen, nodesPen, rootIdx)
			self.makeNetwork(self, Ybus, root, self.battnodes, self.qmin, self.qmax, self.umin, self.umax, Vmin, Vmax, Vtol, v_root)
		else:
			self.battnodes=np.nan, self.qmin=np.nan, self.qmax=np.nan, self.umin=np.nan, self.umax=np.nan
			print('run Network.inputStorage to finish initialization')


	def inputStorage(self, Ybus, battnodes, qmin, qmax, umin, umax):
		# Run when inputing your own configuration of storage and solar units

		self.makeNetwork(self, Ybus, self.root, battnodes, qmin, qmax, umin, umax, self.Vmin, self.Vmax, self.Vtol, self.v_root)
		self.battnodes=battnodes, self.qmin=qmin, self.qmax=qmax, self.umin=umin, self.umax=umax
